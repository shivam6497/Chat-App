...
export default function ChatRoom({
  ...
  return (
    <div className="h-screen overflow-hidden flex" style={{ backgroundColor: 'var(--bg-app)' }}>
      {/* Sidebar: room info + user list */}
      <aside
      ...
      >
        ...
        <div className="mb-6">
          ...
          <button
            ...
            className="flex items-center gap-2 font-bold text-lg border rounded-md px-3 py-2 w-full justify-between transition-colors"
            style={{ borderColor: 'var(--border-field)', color: 'var(--text-primary)' }}
          >
            {roomId}
        ...
        <div className="flex items-center gap-2 mb-2" style={{ color: 'var(--text-muted)' }}>
          <Users size={14} />
          <p className="text-xs uppercase tracking-wide">In Room ({users.length})</p>
        </div>
          ...
          {users.map((u, i) => (
            <li
              ...
              style={{
                ...
                backgroundColor: u === username ? 'var(--bg-field)' : 'transparent',
              }}
            >
              {u} {u === username && <span style={{ color: 'var(--text-muted)' }}>(you)</span>}
            </li>
          ))}
        </ul>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        {/* Mobile header (room code + leave, shown when sidebar is hidden) */}
        <div
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 sm:px-8 py-6 space-y-3">
          ...
          {messages.map((msg, i) => {
            ...
            return (
              <div key={`${msg.ts}-${i}`} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
          ...
          {systemMessages.map((sm, i) => (
            <p key={`sys-${i}`} className="text-xs text-center" style={{ color: 'var(--text-muted)' }}>
              {sm}
            </p>
          ))}
        </div>
